...
export const CarouselView = component$((props: CarouselViewportProps) => {
  ...
  const handlePointerMove$ = $((e: PointerEvent) => {
    if (context.isDraggingSig.value) {
      ...
      const midpoint =
        context.slideRefsArray.value[0].value.offsetWidth / 2 + 50;

      if (context.slideOffsetSig.value < totalWidthSig.value + midpoint) {
        return;
      }
      ...
      for (let i = 0; i < context.slideRefsArray.value.length; i++) {
        ...
        const isWithinBounds =
          absContainerTranslateX >
            slideSlideContainerLeftOffset - halfViewportWidth &&
